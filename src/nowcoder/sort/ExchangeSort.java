...
public class ExchangeSort {
    public static  void   main(String args[]){
       ...
       ExchangeSort exchangeSort=new ExchangeSort();
        int testTime = 500000; //验证次数
        int maxSize = 10;   //数组长度
        int maxValue = 50;   //数组最大值
        boolean succeed = true;
        for (int i = 0; i < testTime; i++) {  //使用对数器进行验证
        ...
        int[] arr = Sort.generateRandomArray(maxSize, maxValue); //构造随机数组
    ...
    public void  BubbingSort(int  []a){  //冒泡排序，时间复杂度O(n^2)  空间复杂度O(1)
        ...
        //第二种方式(依次把最大数的往后排)
        for(int end=a.length-1;end>0;end--){  //只剩最后一个的时候不用排
            for(int j=0;j<end;j++){
                if(a[j]>a[j+1]){
                    Swap(a,j,j+1);
                }
    ...
    public void  InsertSort(int []a){//插入排序,时间复杂度O(n^2)  空间复杂度O(1)
        if(a==null ||a.length<2){
            return;
        }
